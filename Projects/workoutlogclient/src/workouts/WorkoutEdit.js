...
const WorkoutEdit = (props) => {
    ...
    const workoutUpdate = (event, workout) => { //1
        event.preventDefault();
        fetch(`http://localhost:3000/api/log/update/${props.workoutToUpdate.id}`, {
          method: 'PUT',
          body: JSON.stringify({log: {description: editDesc, definition: editDef, result: editRes}}), //2
          headers: new Headers({ //3
            'Content-Type': 'application/json',
            'Authorization': props.token
          })
          }) .then((res) => {
            props.fetchWorkouts(); //4
            props.updateOff();
          })
      }
